...
export default function DashboardScreen({ 
  ...
  return (
    <motion.div
      ...
      animate="visible"
      exit="exit"
      className="absolute inset-0 flex flex-col p-4 sm:p-6 md:p-12 overflow-y-auto no-scrollbar bg-[var(--color-theme-primary)]"
    >
      {/* Header */}
      <motion.div variants={itemVariants} className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl md:text-4xl font-bold text-[var(--color-theme-text)] tracking-tight">
            Dashboard
          </h1>
        ...
        <motion.button
          ...
          className="flex items-center justify-center gap-2 px-6 py-3 bg-[var(--color-theme-card)] text-[var(--color-theme-text)] font-bold rounded-2xl shadow border-2 border-white hover:border-[var(--color-theme-accent)] transition-colors"
        >
          <ShoppingBag size={20} />
